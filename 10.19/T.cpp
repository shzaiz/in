...
void ALL(){
	...
	for(int i=makestr(od,stodd,enod,nod);i!=-1;i = makestr(od,stodd,enod,nod)){
		
		if(i==0) ans=mul(ans,qpow(k-1,enod-stodd,MOD));
		else if(i==233) ans=mul(ans,qpow(k-1,enod-stodd+1,MOD));
		else if(i==1) {
			if(od[stodd]!=od[enod]){
				ans = mul(ans,f[enod-stodd-1][0]); 
			}else{
				ans = mul(ans,f[enod-stodd-1][1]);
			}
		}
	}
