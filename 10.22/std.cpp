...
void dfs(int u, int fa) {
  int an2 = ans2, pvv = -1, bkk = bk, com = 0, an1 = ans1, fstt = fst;
  s[++tp] = alp[u];
  if (u == 1 && s[tp] == 0) {
    ...
    if (combo) {
      if (pv == -1 || fst) {
        ans1 = mul(1, qpow(k - 1, bk - 1));an1 = ans1;
      } else if (s[pv] == s[bk]) {
        ans1 = mul(ans1, f[1][bk - pv - 1]);an1 = ans1;
      } else {
        ans1 = mul(ans1, f[0][bk - pv - 1]);an1 = ans1;
	  }
      finalans ^= ans1;
    } else {
      if (fst) {
        ans1 = qpow(k - 1, tp - 1);an1 = ans1;finalans ^= ans1;
      } else {
        finalans ^= ans1;
      }
    }
...
signed main() {
  ...
  for (int i = 1; i < n; i++) {
    int x;cin >> x;adde(i + 1, x);adde(x, i + 1);
  }
  f[0][0] = 1;
  for (int i = 1; i <= n; i++) {
    f[0][i] = (int)add(f[1][i - 1], mul((k - 2), f[0][i - 1]));f[1][i] = (int)mul((k - 1), f[0][i - 1]);
  }
  dfs(1, 0);
  printf("%lld\n", finalans);
}