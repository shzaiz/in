...
void dfs(int u, int fa) {
  ...
  if (u == 1 && s[tp] == 0) {ans2 = mul(ans2, k);an2 = ans2;combo = 1;finalans ^= mul(ans2, ans1);
  } else if (s[tp] == 0) {if (!combo) {pv = tp - 1;pvv = pv;} else {pvv = pv;}
    ...
    if (combo) {
      ...
      if (fst) {ans1 = qpow(k - 1, tp - 1);an1 = ans1;finalans ^= ans1;} else { finalans ^= ans1;}
    }
