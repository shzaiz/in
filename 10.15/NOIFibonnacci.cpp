...
int n, m, a[maxn], fib[maxn], pre[maxn], sum[maxn * 4], add1[maxn * 4],
    add2[maxn * 4], rfib[maxn];
inline void pushup(int o) { sum[o] = (sum[o << 1] + sum[o << 1 | 1]) % mod; }
inline void pushdown(int o, int l, int r) {
  ...
  sum[o << 1] = (sum[o << 1] +
                 1ll * (pre[mid + 1] - pre[l] + mod) % mod * add1[o] % mod) %
                mod;
  sum[o << 1] = (sum[o << 1] +
                 1ll * (pre[mid] - pre[l - 1] + mod) % mod * add2[o] % mod) %
                mod;
  sum[o << 1 | 1] = (sum[o << 1 | 1] + 1ll * (pre[r + 1] - pre[mid + 1] + mod) %
                                           mod * add1[o] % mod) %
                    mod;
  sum[o << 1 | 1] = (sum[o << 1 | 1] +
                     1ll * (pre[r] - pre[mid] + mod) % mod * add2[o] % mod) %
                    mod;
  add1[o] = add2[o] = 0;
}
...
void update(int o, int l, int r, int ql, int qr, int a1, int a2) {
  if (l >= ql && r <= qr) {
    ...
    sum[o] =
        (sum[o] + 1ll * (pre[r + 1] - pre[l] + mod) % mod * a1 % mod) % mod;
    sum[o] =
        (sum[o] + 1ll * (pre[r] - pre[l - 1] + mod) % mod * a2 % mod) % mod;
    return;
  }
  pushdown(o, l, r);
  int mid = (l + r) >> 1;
